 ...
 */
 class AnimateCheckBox : View, Checkable {
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        val ta = context.obtainStyledAttributes(attrs, R.styleable.AnimateCheckBox)
        val tickColor = ta.getColor(R.styleable.AnimateCheckBox_color_tick, COLOR_TICK)
        mAnimDuration = ta.getInt(R.styleable.AnimateCheckBox_duration, DEF_ANIM_DURATION)
        mFloorColor = ta.getColor(R.styleable.AnimateCheckBox_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED)
        mCheckedColor = ta.getColor(R.styleable.AnimateCheckBox_color_checked, COLOR_CHECKED)
        mUnCheckedColor = ta.getColor(R.styleable.AnimateCheckBox_color_unchecked, COLOR_UNCHECKED)
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.AnimateCheckBox_stroke_width,
                dp2px(context, 0f))
        ta.recycle()
    ...
    override fun setChecked(checked: Boolean) {
        ...
        invalidate()
        if (mListener != null) {
            mListener!!.onCheckedChanged(this@AnimateCheckBox, mChecked)
        }
    }
    ...
    interface OnCheckedChangeListener {
        fun onCheckedChanged(checkBox: AnimateCheckBox, isChecked: Boolean)
    }
    ...
    companion object {
        ...
        const val COLOR_TICK = Color.WHITE
        const val COLOR_UNCHECKED = Color.WHITE
        private val COLOR_CHECKED = Color.parseColor("#FB4846")
        private val COLOR_FLOOR_UNCHECKED = Color.parseColor("#DFDFDF")
        const val DEF_DRAW_SIZE = 25
        const val DEF_ANIM_DURATION = 300
